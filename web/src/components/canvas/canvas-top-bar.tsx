...
function CompactAgentStatus({ status, onClick }: { status: { connected: boolean; agentOnline?: boolean; enabled: boolean; activity: string }; onClick: () => void }) {
    ...
    return (
        <button type="button" className="flex h-8 items-center gap-1.5 text-xs transition hover:opacity-75" style={{ color: status.connected ? "#16a34a" : online ? "#d97706" : theme.node.muted }} onClick={onClick} title={status.connected ? "画布工具桥已就绪，可操作节点" : online ? "Agent 在线，等待工具桥" : "打开本地 Codex 面板"}>
