# 角色部件工坊（Character Atelier）设计文档 v0.3
...
5. **VTON：** 产品化排在 P4；技术 Spike 提前到 P0A（CatVTON 官方口径 bf16 1024×768 <8G 显存、有官方 ComfyUI 节点；许可证 CC BY-NC-SA 非商用，必须记录进能力矩阵）。
...
### 2.3 Windows ComfyUI 实测（2026-07-13）

- 总节点 2284；分割/姿态/打标相关 351。
- **分割/打标可用**：`SAM3Segment` / `SAM2Segment`、`ClothesSegment` / `BodySegment` / `FaceSegment`、`LayerMask: SegformerB2ClothesUltra`、`BiRefNetRMBG`、`OpenposePreprocessor`、`DeepDanbooruCaption`、`Florence2Run`。
...
- Pose 资产 = OpenPose 骨架 + 场景模板 id（对接提示词组合器的场景模板与现有换姿管线）。
...
- 使用独立 `OperationProfile=outpaint`，并区分双模式：
  - **全身重构 `full_body`**：近景/头像默认。**禁止**「缩小脸硬贴 + 高 denoise 蒙版」。正确路径：主图 **pure txt2img 构图优先**（`reference_mode=none` + `denoise=false` + `full_body_rebuild=true`），按**全身竖图比例**重生成（标准约 `1024×1536` / 1.5，舒展约 `1024×1728` / 1.65，上限约 `1024×1792` / 1.75）；上半区无脸自动强构图重试；主图成立后再做脸部二次精修锁身份。**不要**主图直接 FaceID/IPAdapter 近景参考——会把镜头钉成半身或只出下半身。
  - **原图续接 `extend`**：半身/全身已成立时按方向扩展；支持上/下/左/右，主体位置不动；走蒙版 inpaint + 像素回贴；默认 `denoise≈0.68`。近景补头请选向上，不要指望向下续接变全身。
...
- `full_body` 关闭 FaceDetailer，并传 `identity_prompt`；不把节点手动 reference mode 覆盖托管策略。
- 默认继承源图实际 GenerationRecipe，而不是自动添加推荐 LoRA。
- 中文画面要求在提交前经文本模型整理为英文单语；失败时回落英文 fallback。
- 当前状态：双模式代码已按上述口径修正（2026-07-14 畸变修复）；**仍待 BOSS 实图验收**。
...
- 向下 Outpaint 支持全身重构/原图续接双模式：前端扩展画布并生成保护蒙版，网关通过蒙版 inpaint + 像素回贴锁定保护区域；中文需求提交前统一英文化。
...
- **VTON Spike**：家里 5060Ti 16GB 装 CatVTON 官方 Comfy 节点冒烟（CatVTON 官方口径 <8GB 可跑，16GB 显存余量充足）；结论只影响 Reference schema 与 P4 计划，不阻塞主线。
...
| 许可证踩雷 | 能力矩阵记录 license（CatVTON/TryOffDiff 均非商用） |
...
| VTON 轻量化 | CatVTON, arXiv 2407.15886（ICLR 2025）：bf16 1024×768 <8G、官方 Comfy 节点、CC BY-NC-SA | §11 P0A Spike、§6.1 license 字段 |
...
| 项 | 路径/事实 |
|---|---|
| 本设计文档 | `infinite-canvas/docs/character-atelier-design.md`（v0.3） |
