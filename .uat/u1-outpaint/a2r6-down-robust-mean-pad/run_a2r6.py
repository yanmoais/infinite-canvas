...
base_path = out / 'a2r6-down-base-1024x1664.png'
mask_path = out / 'a2r6-down-mask-softseam-96-1024x1664.png'
prompt = (
...
print('submitting A2R6...', flush=True)
t0 = time.time()
...
(out / 'a2r6-down-result.png').write_bytes(img_bytes)
base = Image.open(base_path).convert('RGB')
result = Image.open(io.BytesIO(img_bytes)).convert('RGB')
source_h = 1024
diff = ImageChops.difference(base.crop((0,0,base.width,source_h)), result.crop((0,0,result.width,source_h)))
bbox = diff.getbbox()
